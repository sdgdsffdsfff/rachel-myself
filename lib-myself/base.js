;(function($) {
	...
	*/
	;(function(L) {
		...
		*/
		L.prototype.on = function(eventnames, callback) {
		...
		*/
		L.prototype.trigger = function(eventname, args) {
		...
		*/
		L.prototype.off = function(eventname, keys) {
			if (!list[eventname]) return;
			var i, event;
			...
			for (i = 0; i < list[eventname].length; i++) {
				...
				$.inArray(event.key, keys) && (list[eventname][i] = undefined);
			}
		}
		...
		* @param [fn] callback 回调函数
		*/
		L.prototype.once = function(eventname, callback) {
			var self = this;
